 ...
 * Configuration from API with fallback
 */
export default function HeaderCore() {
  ...
  useEffect(() => {
    getHeaderConfig().then(setConfig).catch(() => {});
  }, []);
  ...
  const enabledSocials = config.socials?.filter(s => s.enabled) || [];
      ...
      <header className="ys-header" data-testid="header-core">
        ...
        <div className="ys-container ys-header-top">
          ...
          <div className="ys-header-phones hide-on-mobile">
            {config.phones?.slice(0, 2).map((phone, i) => (
              <a 
                key={i} 
