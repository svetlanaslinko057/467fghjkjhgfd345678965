...
const validateName = (name) => {
  // Allow only letters, spaces, apostrophes, hyphens (for Ukrainian/Russian names)
  const nameRegex = /^[a-zA-Zа-яА-ЯіІїЇєЄґҐ'\-\s]+$/;
  if (!name.trim()) return { valid: false, error: "Вкажіть ім'я та прізвище" };
  if (name.trim().length < 3) return { valid: false, error: "Ім'я занадто коротке" };
  if (name.trim().split(' ').length < 2) return { valid: false, error: "Вкажіть ім'я та прізвище" };
  if (!nameRegex.test(name)) return { valid: false, error: "Ім'я може містити тільки літери" };
  return { valid: true };
};
...
const CheckoutV3 = () => {
  ...
  const handlePhoneChange = (e) => {
    ...
    if (validation.valid && validation.operator) {
      ...
      setPhoneOperator('');
      if (formatted.replace(/\D/g, '').length >= 5) {
  ...
  const handleNameChange = (e) => {
    ...
    if (value.length > 2) {
      ...
      setErrors(prev => ({ ...prev, full_name: validation.error || '' }));
    } else {
  ...
  const validateStep = (step) => {
    if (step === 1) {
      ...
      // Validate name
      const nameValidation = validateName(customerData.full_name);
                  ...
                  <div className="mt-6 space-y-5">
                    ...
                    {(customerType === 'new' || isAuthenticated) && (
                      <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            ...
                            <Input
                              ...
                              required
                              className={`mt-1 h-12 ${errors.full_name ? 'border-red-500 focus:ring-red-500' : ''}`}
                          ...
                          <div>
                            ...
                            <div className="relative mt-1">
                              ...
                              <Input
                                ...
                                required
                                className={`pl-11 h-12 ${errors.phone ? 'border-red-500 focus:ring-red-500' : ''}`}
                        ...
                        {/* Email field */}
                        <div>
                          <Label htmlFor="email">Email (для підтвердження замовлення)</Label>
                          <div className="relative mt-1">
